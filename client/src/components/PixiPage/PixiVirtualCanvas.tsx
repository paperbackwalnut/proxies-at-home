...
import { pixiSingleton, resetPixiSingleton, setPixiApp } from "./pixiSingleton";
import {
  calculateHoloAnimation,
  type HoloAnimationStyle,
} from "./holoAnimation";
...
function PixiVirtualCanvasInner({
  ...
  const darkenApplyToScryfall = useSettingsStore(
    (s) => s.darkenApplyToScryfall
  );
  ...
  const darkenApplyToCardbacks = useSettingsStore(
    (s) => s.darkenApplyToCardbacks
  );
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      for (const { card } of cardsRef.current) {
        const overrides = card.overrides;
        if (
          overrides?.holoEffect &&
          overrides.holoEffect !== "none" &&
          overrides.holoAnimation &&
          overrides.holoAnimation !== "none"
        ) {
  ...
  const createTexture = useCallback(
    async (blob: Blob, cacheKey: string): Promise<Texture | null> => {
    ...
    const updateSprites = async () => {
      ...
      for (let i = 0; i < cards.length; i++) {
        ...
        if (!spriteData) {
          ...
          if (backBlob) {
            backTexture =
              (await createTexture(backBlob, `back-${uuid}`)) ?? undefined;
            if (isStale()) return;
          }

          // Determine initial texture
          const initialTexture =
            isFlipped && backTexture ? backTexture : frontTexture;
        ...
        const activeBleedMm =
          isFlipped && cards[i].backBleedMm !== undefined
            ? cards[i].backBleedMm!
            : cards[i].bleedMm;
        ...
        const overrides = isFlipped
          ? (backOverrides ?? card.overrides)
          : card.overrides;

        // Calculate effective global darken mode for this specific card
        const currentSource = isFlipped
        ...
        const effectiveMode = useGlobal
          ? effectiveGlobalDarkenMode
          : (overrides?.darkenMode ?? effectiveGlobalDarkenMode);
...
const CARD_SHALLOW_KEYS: (keyof CardWithGlobalLayout)[] = [
  ...
  "backBleedMm",
  "frontImageId",
  "backImageId", // Important for detecting artwork changes even if blob refs stay same
];

function arePropsEqual(
  prevProps: PixiVirtualCanvasProps,
  nextProps: PixiVirtualCanvasProps
): boolean {
