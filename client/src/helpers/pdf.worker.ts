...
function computeEffectCacheKey(
  imageId: string,
...
async function cacheEffectBlob(
  imageId: string,
  overrides: CardOverrides,
  blob: Blob,
  dpi: number
...
function createFullPageGuidesCanvas(
  ...
  if (cutLineStyle === "none" || guideWidthPx <= 0 || cardLayouts.length === 0)
    return null;
  ...
  const gridWidthPx =
    ...
    Math.max(0, colWidths.length - 1) * spacingPx;
  const gridHeightPx =
    rowHeights.reduce((sum, h) => sum + h, 0) +
    Math.max(0, rowHeights.length - 1) * spacingPx;
...
// Cricut PTC reg mark
const CRICUT_MARK_OFFSET_X_MM = 10.626;
const CRICUT_MARK_OFFSET_Y_MM = 3.351;
const CRICUT_ARM_THICKNESS_MM = 1.475;
const CRICUT_ARM_LENGTH_MM = 22.391;
const CRICUT_GAP_MM = 1.583;
...
// Draw cricut reg marks
    function drawCricutRegistrationMarks(
    ctx: OffscreenCanvasRenderingContext2D,
    pageWidthPx: number,
    pageHeightPx: number,
    dpi: number
): void {
    const s = MM_TO_PX(1, dpi);
    ctx.fillStyle = '#000000';
    ...
    ctx.lineTo(4.8278*s, 12.101*s);
    ctx.closePath();
    ctx.fill();

    // TOP-RIGHT: double-line L polygon
    ctx.beginPath();
    ctx.moveTo(pageWidthPx - 4.826*s,  36.025*s);
    ctx.lineTo(pageWidthPx - 3.351*s,  36.025*s);
    ctx.lineTo(pageWidthPx - 3.351*s,  10.625*s);
    ctx.lineTo(pageWidthPx - 28.825*s, 10.625*s);
    ctx.lineTo(pageWidthPx - 28.825*s, 12.100*s);
    ctx.lineTo(pageWidthPx - 4.826*s,  12.100*s);
    ctx.closePath();
    ctx.fill();

    // BOTTOM-LEFT: 2 rects + triangle
    ctx.fillRect(3.3514*s, pageHeightPx - 13.6342*s - 22.3908*s, 1.4746*s, 22.3908*s);
    ctx.fillRect(6.4107*s, pageHeightPx - 10.6264*s - 1.4746*s, 22.4162*s, 1.4746*s);
    ctx.beginPath();
    ctx.moveTo(3.3532*s, pageHeightPx - 11.3637*s);
    ctx.lineTo(4.8278*s, pageHeightPx - 10.6264*s);
    ctx.lineTo(4.8278*s, pageHeightPx - 12.101*s);
    ctx.closePath();
    ctx.fill();

    // BOTTOM-RIGHT: double-line L polygon
    ctx.beginPath();
    ctx.moveTo(pageWidthPx - 4.826*s,  pageHeightPx - 36.025*s);
    ctx.lineTo(pageWidthPx - 3.351*s,  pageHeightPx - 36.025*s);
    ctx.lineTo(pageWidthPx - 3.351*s,  pageHeightPx - 10.625*s);
    ctx.lineTo(pageWidthPx - 28.825*s, pageHeightPx - 10.625*s);
    ctx.lineTo(pageWidthPx - 28.825*s, pageHeightPx - 12.100*s);
    ctx.lineTo(pageWidthPx - 4.826*s,  pageHeightPx - 12.100*s);
    ctx.closePath();
    ctx.fill();
}
/**
...
self.onmessage = async (event: MessageEvent) => {
  try {
    ...
    const pageWidthPx =
      pageSizeUnit === "in"
        ? IN_TO_PX(pageWidth, DPI)
        : MM_TO_PX(pageWidth, DPI);
    const pageHeightPx =
      pageSizeUnit === "in"
        ? IN_TO_PX(pageHeight, DPI)
        : MM_TO_PX(pageHeight, DPI);
    ...
    const layoutsMm = computeCardLayouts(
      pageCards,
      sourceSettings,
      bleedEdge ? bleedEdgeWidthMm : 0
    );
    const { colWidthsMm, rowHeightsMm } = computeGridDimensions(
      layoutsMm,
      columns,
      rows,
      cardSpacingMm
    ...
    debugLog(
      `[PDF Worker] Processing ${pageCards.length} cards with concurrency limit ${MAX_CONCURRENT_CARDS} `
    );
    const preparedCards = await processWithConcurrency(
      pageCards,
      async (card: CardOption, idx: number): Promise<PreparedCard> => {
        const col = idx % columns;
        const row = Math.floor(idx / columns);
        ...
        const imageInfo = card.imageId
          ? imagesById.get(card.imageId)
          : undefined;

        // Determine effective mode and settings
        const useGlobalSettings =
          card.overrides?.darkenUseGlobalSettings ?? true;
        ...
        const effectiveDarkenMode =
          (useGlobalSettings ? effectiveGlobalDarkenMode : cardDarkenMode) ??
          effectiveGlobalDarkenMode;

        // Helper to get the correct generated blob based on the requested darken mode
        const extractDarkenedBlob = (
          result: Partial<WorkerSuccessResponse>,
          mode: string
        ): Blob | undefined => {
          switch (mode) {
            case DarkenMode.DarkenAll:
              return result.exportBlobDarkenAll ?? result.exportBlob;
            case DarkenMode.ContrastEdges:
              return result.exportBlobContrastEdges ?? result.exportBlob;
            case DarkenMode.ContrastFull:
              return result.exportBlobContrastFull ?? result.exportBlob;
            default:
              return result.exportBlob;
          }
        };

        // Resolve darken parameters with fallback to global settings
        const resolveParam = (
          cardVal: number | undefined,
          globalVal: number
        ) =>
          card.overrides && !useGlobalSettings
            ? (cardVal ?? globalVal)
            : globalVal;

        const r_darkenAutoDetect =
          card.overrides && !useGlobalSettings
        ...
        const isAutoContrast =
          r_darkenAutoDetect &&
          (effectiveDarkenMode === DarkenMode.ContrastEdges ||
            effectiveDarkenMode === DarkenMode.ContrastFull);

        const darkenOpts = {
          darkenThreshold: resolveParam(
            card.overrides?.darkenThreshold,
            darkenThreshold ?? 30
          ),
          darkenContrast: isAutoContrast
            ? 2.0
            : resolveParam(
                card.overrides?.darkenContrast,
                darkenContrast ?? 2.0
              ),
          darkenEdgeWidth: resolveParam(
            card.overrides?.darkenEdgeWidth,
            darkenEdgeWidth ?? 0.1
          ),
          darkenAmount: resolveParam(
            card.overrides?.darkenAmount,
            darkenAmount ?? 1.0
          ),
          darkenBrightness: isAutoContrast
            ? -50
            : resolveParam(
                card.overrides?.darkenBrightness,
                darkenBrightness ?? -50
              ),
          darkenAutoDetect: r_darkenAutoDetect,
        };

        const needsCustomDarken =
          effectiveDarkenMode !== DarkenMode.None &&
        ...
        const existingBleedMm =
          getEffectiveExistingBleedMm(card, { withBleedSourceAmount }) ?? 0;
        const targetBleedMm = bleedEdge
          ? getCardTargetBleed(card, sourceSettings, bleedEdgeWidthMm)
          : 0;
        ...
        const isCacheValid =
          selectedExportBlob && (isCardback || imageInfo?.exportDpi === DPI);
        ...
        const customDarkenKey = needsCustomDarken
          ? `-${darkenOpts.darkenThreshold}-${darkenOpts.darkenContrast}-${darkenOpts.darkenEdgeWidth}-${darkenOpts.darkenAmount}-${darkenOpts.darkenBrightness}`
          : "";
        const cacheKey = card.imageId
          ? `${card.imageId}-${targetBleedMm.toFixed(2)}-${effectiveDarkenMode}${customDarkenKey}-${DPI}`
          : null;
        ...
        if (isCacheValid) {
          ...
          debugLog(
            `[PDF Worker] Card ${idx}: Using cached blob, size = ${selectedExportBlob!.size} `
          );
          let bitmap = await createImageBitmap(selectedExportBlob!);
          debugLog(
            `[PDF Worker] Card ${idx}: Created bitmap ${bitmap.width}x${bitmap.height} `
          );

          if (needsCustomDarken) {
            debugLog(
              `[PDF Worker] Card ${idx}: Applying custom darken settings`
            );
            const darkenCanvas = await renderBleedCanvasDirect(
              bitmap,
              bitmap.width,
              bitmap.height,
              {
          ...
          if (hasAdvancedOverrides(card.overrides)) {
            ...
            if (cachedEffectBlob) {
              ...
              const params = overridesToRenderParams(
                card.overrides!,
                effectiveDarkenMode as DarkenMode
              );
              params.dpi = DPI;
              const renderedBlob = await renderCardWithOverridesWorker(
                bitmap,
                params
              );
              bitmap.close();
          ...
          if (actualImageBleedPx !== imageBleedPx) {
            debugLog(
              `[PDF Worker] Card ${idx}: Cached blob bleed mismatch! Expected ${imageBleedPx} px, got ${actualImageBleedPx} px`
            );
          ...
          if (cacheKey && canvasCache.has(cacheKey)) {
            ...
            let src = imageInfo?.originalBlob
              ? URL.createObjectURL(imageInfo.originalBlob)
              : imageInfo?.sourceUrl;
            ...
            try {
              if (card.imageId === "cardback_builtin_blank") {
                ...
                debugLog(
                  `[PDF Worker] Card ${idx}: Blank canvas dimensions: ${cardWidthWithBleed}x${cardHeightWithBleed} `
                );
                const blankCanvas = new OffscreenCanvas(
                  cardWidthWithBleed,
                  cardHeightWithBleed
                );
                ...
                if (!cardCtx) {
                  throw new Error(
                    `Failed to get 2d context for blank canvas ${cardWidthWithBleed}x${cardHeightWithBleed} `
                  );
                }
                debugLog(
                  `[PDF Worker] Card ${idx}: Got 2d context, filling white`
                );
                ...
                debugLog(
                  `[PDF Worker] Card ${idx}: Creating placeholder(no source)`
                );
                const cardBleedPx = cardLayout.bleedPx;
                const cardWidthWithBleed = contentWidthInPx + 2 * cardBleedPx;
                const cardHeightWithBleed = contentHeightInPx + 2 * cardBleedPx;
                const placeholderCanvas = new OffscreenCanvas(
                  cardWidthWithBleed,
                  cardHeightWithBleed
                );
                ...
                if (!cardCtx) {
                  throw new Error(
                    `Failed to get 2d context for placeholder canvas`
                  );
                }
                cardCtx.fillStyle = "white";
                cardCtx.fillRect(0, 0, cardWidthWithBleed, cardHeightWithBleed);
                ...
                cardCtx.fillText(
                  "Image not found",
                  cardWidthWithBleed / 2,
                  cardHeightWithBleed / 2
                );
                finalCardCanvas = placeholderCanvas;
              } else {
                // Fetch image
                debugLog(
                  `[PDF Worker] Card ${idx}: Fetching image from ${src.substring(0, 50)}...`
                );
                ...
                const needsBleedChange =
                  Math.abs(existingBleedMm - targetBleedMm) > 0.01;
                ...
                if (effectiveMode === "none") {
                  ...
                  const result = await processExistingBleedWebGL(
                    img,
                    targetBleedMm,
                    {
                      unit: "mm",
                  ...
                  const finalBlob =
                    extractDarkenedBlob(result, effectiveDarkenMode) ??
                    result.exportBlob;
                  if (!finalBlob)
                    throw new Error(
                      "Missing exportBlob from processExistingBleedWebGL"
                    );
                  ...
                  finalCardCanvas = await generateBleedCanvasWebGL(
                    img,
                    targetBleedMm,
                    {
                      unit: "mm",
                      dpi: DPI,
                      inputBleed:
                        card.hasBuiltInBleed && existingBleedMm > 0
                          ? existingBleedMm
                          : 0,
                      darkenMode: effectiveDarkenMode,
                      ...darkenOpts,
        ...
        if (
          cacheKey &&
          trimmedCanvas instanceof OffscreenCanvas &&
          !fromCanvasCache
        ) {
        ...
        return {
          ...
          centerOffsetY:
            trimmedHeight === imageCardHeightPx ? centerOffsetY : 0,
    ...
    for (const prepared of preparedCards) {
      ...
      if (!prepared.isBlank) {
        ...
        ctx.rect(
          prepared.x,
          prepared.y,
          prepared.cardWidthPx,
          prepared.cardHeightPx
        );
        ctx.clip();
        ctx.drawImage(
          prepared.canvas,
          prepared.x + prepared.centerOffsetX,
          prepared.y + prepared.centerOffsetY,
          prepared.imageCardWidthPx,
          prepared.imageCardHeightPx
        );
        ctx.restore();
        ...
        if (perCardGuideCanvas) {
          ...
          ctx.drawImage(
            perCardGuideCanvas,
            prepared.x + guideOffsetX,
            prepared.y + guideOffsetY
          );
          ctx.restore();
        }
      }
    ...
    if (registrationMarks && registrationMarks !== "none") {
      if (registrationMarks === "cricut") {
        drawCricutRegistrationMarks(ctx, pageWidthPx, pageHeightPx, DPI);
      } else {
        drawSilhouetteRegistrationMarks(
          ctx,
          pageWidthPx,
          pageHeightPx,
          DPI,
    ...
    try {
      ...
      const canvasMb = Math.round(
        (pageWidthPx * pageHeightPx * 4) / 1024 / 1024
      );
    ...
    if (error instanceof Error) {
      ...
      self.postMessage({
        error: "An unknown error occurred in the PDF worker.",
        pageIndex: event.data.pageIndex,
      });
    }
  }
